...
func TestAcquireSBOMRejectsBelow15(t *testing.T) {
	var warn bytes.Buffer
	_, err := acquireSBOM(writeSBOM(t, cdxDoc("1.4")), &warn)
	if err == nil || !strings.Contains(err.Error(), "below the 1.5 minimum") {
		t.Fatalf("want below-1.5 rejection, got %v", err)
	}
}

// cargo-cyclonedx maxes at CycloneDX 1.5 (its 1.6 support is upstream #769). 1.5 is
// accepted with an up-convert warning and normalized to 1.6.
func TestAcquireSBOMAccepts15WithWarnAndUpconverts(t *testing.T) {
	var warn bytes.Buffer
	out, err := acquireSBOM(writeSBOM(t, cdxDoc("1.5")), &warn)
	if err != nil {
		t.Fatalf("acquireSBOM: %v", err)
	}
	if !strings.Contains(warn.String(), "up-converting to 1.6") {
		t.Errorf("want stderr 1.5 up-convert warning, got %q", warn.String())
	}
