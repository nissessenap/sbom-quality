...
// supplier(proxy)/license/vcs from the upstream registry — in practice this
// lands on Go components (parlay no-ops on entries it can't resolve, so an
// image-only run is harmless). Checksums are not enrich's job (no Go registry
